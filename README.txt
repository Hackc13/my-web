...
Fixed:
- Account button is now smaller.
- Clicking Account now opens the dropdown.
- Email is kept as tooltip, but button text stays Account to keep size small.
- Edit Profile opens edit-profile.html.
- Edit Password opens edit-password.html.

Upload full ZIP and replace old files.


FAVICON / TAB LOGO UPDATE

Fixed:
- Browser tab/default grey icon replaced with CrazyHub logo icon.
...
Important:
After upload, browser may cache favicon strongly.
Open Incognito or clear browser cache. On Chrome, also try:
https://your-site-url/favicon.ico
then refresh.


CUSTOM 404 PAGE UPDATE

Added:
- 404.html

Design:
- Dotted animated 404 style like the provided video
- Light center card
- Mouse hover / magnetic dots movement
- CrazyHub logo
- Homepage link

GitHub Pages:
- 404.html works automatically when a page does not exist.

Firebase Hosting:
- Add this rewrite in firebase.json if needed:
  "rewrites": [
    { "source": "**", "destination": "/404.html" }
  ]
But only use this for fallback pages; if your app uses normal index routing, keep your existing hosting setup.


404 EXACT VIDEO STYLE UPDATE

Updated 404.html to match the provided video more closely:
- Plain light background
- White/cream centered card
- Black border and shadow
- Big black dotted 404
...
- Huge dotted 404
- Dots move away from cursor
- Text below exactly like the reference


404 SOFT DOTS UPDATE

Changed:
- Reduced bold/heavy dotted 404 look.
- Dots are smaller and lighter.
- 404 shape is less thick.


404 REFERENCE MATCH UPDATE

Updated 404.html again to match the uploaded screenshot:
- Thinner dotted 404
- Smaller/lighter dots
- Light grey background
- No card or border
- Text below in grey
- Cursor still pushes dots away


404 FOUR-LINE DOT UPDATE

Changed:
- 404 stroke is thicker now.
- Each number shape has around 4 dot rows/lines like the reference.
- Dot size remains small, not oversized.


404 EXACT REFERENCE DOTS UPDATE

Updated 404.html using dot positions extracted from the user's reference image.
- Exact dot layout from the screenshot
- 4-line thickness preserved
- Light grey background
- Cursor/mouse still pushes dots away
- Total dots used: 443


404 CLEAN LAST 4 UPDATE

Fixed:
- Removed unwanted extra dot parts from the last 4.
- Kept first 4 and 0.
- Rebuilt last 4 using only the clean first 4 shape.


404 ERROR READY UPDATE

This version is prepared to show 404.html when a wrong/missing page is opened.

Included:
- 404.html
- firebase.json for Firebase Hosting
- .htaccess for Apache/Namecheap cPanel hosting
- _redirects for Netlify
- vercel.json for Vercel

GitHub Pages:
- 404.html works automatically when it is in the root folder.

Firebase Hosting:
- Deploy with firebase.json included.
- Keep 404.html in the root folder.

Namecheap / cPanel hosting:
- Upload .htaccess and 404.html to public_html.
- Wrong pages will open 404.html.

Important:
Upload the full ZIP. Do not upload only index.html.


SOFTWARE TAB UPDATE
- Added LI Editor card to the Software tab.
- Clicking LI Editor opens software/lifeinvader/index.html.
- The LifeInvader editor uses the uploaded policy rules.
